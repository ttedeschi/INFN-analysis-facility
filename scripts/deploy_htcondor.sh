...
helm repo update
helm install condor infnAF/htcondor --set masterPublicIp=$1 --set namespace=default --set autoscaling.enabled=true --set imageSchedd.repository=ttedesch/htcondor-submit --set imageSchedd.tag=credmon --kubeconfig /etc/kubernetes/admin.conf
